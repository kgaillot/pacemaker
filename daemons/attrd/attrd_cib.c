...
write_attribute(attribute_t *a, bool ignore_delay)
{
    ...
    // Private attributes (all in standalone mode) are not written to CIB
     ...
     * which this is, and we know the node XML ID.
     */
    ...
    while (g_hash_table_iter_next(&iter, NULL, (gpointer *) &v)) {
        ...
        if (pcmk_is_set(v->flags, attrd_value_remote)) {
            ...
            /* If a value was removed, we should already have the XML ID used to
             * write it in the first place. If not, it may be an attribute with
             * a write delay that was written just before the writer's shutdown.
            ...
            if (uuid == NULL) {
                ...
                node_xml_id = crm_peer_uuid(peer);
            }
