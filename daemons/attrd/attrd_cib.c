...
drop_values_in_deletion(xmlNode *xml, void *data)
{
     ...
     * entire node_state was deleted; if no matches[3], entire
     * transient_attributes was deleted)
     */
...
write_attribute(attribute_t *a, bool ignore_delay)
{
    ...
    while (g_hash_table_iter_next(&iter, NULL, (gpointer *) &v)) {
        ...
        if (pcmk_is_set(v->flags, attrd_value_remote)) {
             ...
             * purged node.
             */
            peer = pcmk__search_node_caches(v->nodeid, v->nodename, NULL,
