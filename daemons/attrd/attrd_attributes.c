...
/*!
 * \internal
 * \brief Drop removed attribute values as indicated by given function
 *
 * Drop all node attribute values marked as removed that a given function
 * indicates should be, based on the XML ID of an element that was removed from
 * the CIB.
 *
 ...
 *                      the value (but keep checking the attribute's other
 *                      values) if it returns 1, or drop value and stop checking
 *                      that attribute's further values if it returns 2
 */
static void
                                ...
                                const char *))
{
    ...
    while (g_hash_table_iter_next(&attr_iter, NULL, (gpointer *) &a)) {
        ...
        if (pcmk_is_set(a->flags, attrd_attr_is_private)) {
...
{
    drop_removed_values(cib_id, set_type, set_matches);
}
